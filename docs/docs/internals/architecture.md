...
| Daemon → TWS / Gateway | Clean-room TWS wire protocol over TCP, optionally TLS | A primary interactive connection, plus a breadth connection with its own client ID and rate budget. On a ready breadth connection, a historical-farm warning permits serial recovery reads, each with a two-minute total budget. A failed read stops the pass and backs off from one to fifteen minutes; a successful read admits only the next serial request. Retries rotate past the failed constituent, and observed coverage can publish once the existing threshold is met even if another constituent fails. API or TWS/server loss still stops all reads. Provider warnings, session checks, retained progress and publication coverage gates remain in force. |
...
| Wikipedia S&P 500 list | Scheduled HTTPS refresh | Breadth constituent membership, with a validated SQLite projection and embedded fallback, plus each constituent's GICS sector for portfolio allocation, held in memory over an embedded baseline. |
