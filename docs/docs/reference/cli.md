...
`canary` is one binary with 32 subcommands, listed here in registry order. `canary status` comes first on purpose: when anything else fails, it is the command that says whether the daemon is running and the gateway is reachable. Running `canary` with no arguments prints the grouped command summary.
...
| [`canary data`](#canary-data) | Markets | Canary-owned source health and bounded required-feed checks | `read-only` | yes |
...
| [`canary setup`](#canary-setup) | System | Configure a local client, app host, or shared broker reporting | `local` | CLI only |
...
Canary-owned source health and bounded required-feed checks.

Guard `read-only`. Also available as an MCP tool.

```text
canary data health [--offset N --limit N --revision ID] [--json]
canary data check [--json]
```

Subcommands: `health`, `check`.
...
```text
canary calendar [--market us|us-options|de|uk|jp|hk] [--date YYYY-MM-DD] [--at RFC3339] [--days N] [--json]
```
...
| Flag | Takes a value | Allowed values |
|------|---------------|----------------|
| `--market` | yes | `us`, `us-options`, `de`, `uk`, `jp`, `hk` |
| `--date` | yes | - |
| `--at` | yes | - |
| `--days` | yes | - |
| `--json` | no | - |
