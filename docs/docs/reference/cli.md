...
|---------|---------|-------|----------|
| [`ibkr status`](#ibkr-status) | Daemon + gateway health (run this first if anything fails) | `read-only` | yes |
| [`ibkr account`](#ibkr-account) | Account summary snapshot (NLV, BP, cash, margin, daily P&L) | `read-only` | yes |
| [`ibkr positions`](#ibkr-positions) | List open positions (stocks + options) | `read-only` | yes |
| [`ibkr quote`](#ibkr-quote) | Snapshot or stream quotes for symbols / option contracts | `read-only` | yes |
| [`ibkr watch`](#ibkr-watch) | Local watchlist symbols; add/remove/clear offline or quote the list live | `local` | yes |
| [`ibkr calendar`](#ibkr-calendar) | Official market sessions for US equities, US options, and Xetra | `read-only` | yes |
| [`ibkr chain`](#ibkr-chain) | Option chain table or expiry list | `read-only` | yes |
...
| [`ibkr recon`](#ibkr-recon) | Post-trade reconciliation: broker statement flows vs the declared capital ledger | `confirm` | CLI only |
...
| [`ibkr update`](#ibkr-update) | Self-update the ibkr binary from the latest GitHub release | `confirm` | CLI only |
| [`ibkr restart`](#ibkr-restart) | Gracefully restart the daemon and any running app process | `confirm` | CLI only |
| [`ibkr version`](#ibkr-version) | Print version, commit, build date | `local` | CLI only |
