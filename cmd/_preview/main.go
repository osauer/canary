...
func fixtureAccount() *rpc.AccountResult {
	return &rpc.AccountResult{
		...
		BaseCurrency:         "EUR",
		NetLiquidation:       1250000.00,
		BuyingPower:          4800000.00,
		AvailableFunds:       1100000.00,
		ExcessLiquidity:      1100000.00,
		TotalCash:            150000.00,
		MaintenanceMargin:    25000.00,
		InitialMargin:        31000.00,
		GrossPositionValue:   60000.00,
		UnrealizedPnL:        3113.50,
		RealizedPnL:          0,
		Cushion:              0.98,
		LookAheadInitMargin:  31000.00,
		LookAheadMaintMargin: 25000.00,
		LookAheadAvailable:   1100000.00,
		LookAheadExcess:      1100000.00,
		DailyPnL:             f64(2340.00),
		PnLUnrealizedTotal:   f64(25000.00),
		PnLRealizedTotal:     f64(5000.00),
		CurrencyExposure: []rpc.CurrencyExposure{
			{Currency: "USD", NetLiquidationCcy: 540000.00, ExchangeRate: 0.9000, NetLiquidationBase: 486000.00},
			{Currency: "GBP", NetLiquidationCcy: 10000.00, ExchangeRate: 1.1500, NetLiquidationBase: 11500.00},
		},
...
func fixturePositions() *rpc.PositionsResult {
	...
	return &rpc.PositionsResult{
		...
		// Summary values are derived from the rows above so the rendered
		// table and summary reconcile: deltas 202 (AAPL) + 405 (NVDA) − 94
		// (SPY), dollar delta against marks (SPY spot 583.18), greeks as
		// Σ greek × qty × 100, FX = the 59,161 USD book in EUR × 1%.
		Portfolio: &rpc.PositionsPortfolio{
			EffectiveDelta:      f64(513.0),
			DollarDelta:         f64(39138.62),
			DollarDeltaCurrency: "USD",
			DailyTheta:          f64(-121.00),
			DailyThetaCurrency:  "USD",
			Gamma:               f64(19.1),
			Vega:                f64(654.00),
			GreeksCoverage:      5,
			GreeksTotal:         5,
			FXSensitivityPerPct: f64(-546.62),
			FXBaseCurrency:      "EUR",
		},
		AsOf: time.Date(2026, 5, 13, 14, 32, 18, 0, time.UTC),
...
func fixtureSize() *risk.SizeResult {
	tgt, r, reward, be := 217.50, 2.0, 25000.0, 1.0/3.0
	return &risk.SizeResult{
		Symbol: "AAPL", Side: "long",
		Entry: 207.50, Stop: 202.50, Target: &tgt,
		RiskPct: 1.0, Lot: 1, FX: 1.0,
		NLV: 1250000.00, BaseCurrency: "EUR",
		RiskBase: 12500.00, RiskQuote: 12500.00,
		PerShareRisk: 5.00,
		Shares:       2500, Notional: 518750.00, MaxLoss: 12500.00,
...
func fixtureRegime() *rpc.RegimeSnapshotResult {
	...
	r := &rpc.RegimeSnapshotResult{
		...
		Summary: rpc.RegimeSummary{
			...
			Evidence:          "4 green clusters / 1 yellow cluster / 1 red cluster",
			IndicatorEvidence: "6 green / 1 yellow / 1 red",
		...
		HYGSPYDivergence: rpc.RegimeHYGSPYDivergence{
			RegimeIndicatorMeta: rpc.RegimeIndicatorMeta{
				Band:       "green",
				BandReason: "HYG holding above its 50-day",
		...
		USDJPY: rpc.RegimeUSDJPY{
			RegimeIndicatorMeta: rpc.RegimeIndicatorMeta{
				Band:       "green",
				BandReason: "yen stable on the week",
			...
			Symbol:       "USD.JPY",
			Last:         f64(159.8700),
			Close7DAgo:   f64(159.2100),
			WeeklyChange: f64((159.87 - 159.21) / 159.21 * 100),
			DataType:     rpc.MarketDataLive,
	...
	// Without an authority projection the renderer treats the snapshot as
	// health-unavailable and demotes the whole read to retained context.
	lastSuccess := now.Add(-time.Minute)
	r.AuthorityHealth = &rpc.RegimeAuthorityHealth{
		Status:                rpc.RegimeAuthorityFresh,
		LastSuccessAt:         &lastSuccess,
		LastSuccessAgeSeconds: ptrInt64(60),
	}
