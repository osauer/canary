...
func printMCPUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "The server exposes no-broker-write canary_* tools plus the canary://quote/{symbol}")
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w, "The monitor profile exposes only canary_stress and canary_status for low-token")
