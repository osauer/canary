...
// sectorsPath receives the GICS sector per ticker from the same table.
const sectorsPath = "internal/breadth/spx/sectors_data.go"
...
func main() {
	...
	symbols, sectors, asOf, err := spx.FetchAndParseWithSectors(ctx, url)
	if err != nil {
	...
	// tree clean.
	current, _ := spx.MemberList()
	currentSectors, _ := spx.SectorList()
	if slices.Equal(current, symbols) && maps.Equal(currentSectors, sectors) {
		fmt.Fprintf(os.Stderr, "Membership and sectors unchanged (%d symbols); leaving %s and its as-of stamp alone\n", len(symbols), outputPath)
		return
	}
	if len(sectors) < spx.MinMembers {
		log.Fatalf("sanity check: parsed %d sectors for %d symbols (GICS Sector column may have moved)", len(sectors), len(symbols))
	}
	...
	src, err = renderSectorsData(sectors)
	if err != nil {
		log.Fatalf("render sectors_data.go: %v", err)
	}
	if err := os.WriteFile(sectorsPath, src, 0o644); err != nil {
		log.Fatalf("write %s: %v", sectorsPath, err)
	}
	fmt.Fprintf(os.Stderr, "Wrote %d symbols to %s and %d sectors to %s (as-of %s)\n", len(symbols), outputPath, len(sectors), sectorsPath, asOf.Format("2006-01-02"))
}

// renderSectorsData formats the ticker → GICS sector map as a gofmt-clean
// Go source file, sorted by ticker so re-runs are byte-stable.
func renderSectorsData(sectors map[string]string) ([]byte, error) {
...
// sp500Sectors is the GICS sector Wikipedia lists for each S&P-500
// constituent, observed together with sp500Members at sp500AsOf.
var sp500Sectors = map[string]string{
`)
	for _, t := range slices.Sorted(maps.Keys(sectors)) {
		fmt.Fprintf(&b, "\t%q: %q,\n", t, sectors[t])
	}
	b.WriteString("}\n")
