...
function buildSyntheticSnapshot() {
  ...
  return {
    ...
    account: {
      ...
      daily_pnl_observation: { status: "ok", session_key: "2026-07-30", as_of: asOf },
      ...
      as_of: asOf,
      authority: {
        scope: { account_id: accountID, account_mode: "paper" },
    ...
    brief: {
      ...
      narrative: {
        ...
        ready: [
          ...
          { runs: [
            ...
            { text: " across 1 long option leg; hedge cost EUR -18.40 per day. No protection proposals are staged. Process folds clean: policy pins match, the monthly pulse is not due, and no held-name events." },
          ] },
        ],
      ...
      review: {
        ...
        rules: { ...briefState("All current rulebook checks pass."), pass: 14, watch: 0, act: 0, unknown: 0 },
    ...
    ].map((source) => [source, { updated_at: asOf, last_success_at: asOf, state: "current" }])),
    updated_at: asOf,
  };
}
