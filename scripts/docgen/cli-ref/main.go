...
var commandNotes = map[string]string{
	...
	"purge": "`ibkr purge SYMBOL`, `ibkr purge --all`, and `purge execute` close live positions through the broker. " +
		"They carry the same gate as `ibkr order`: an explicit human instruction for that exact action, in that moment. " +
...
func render(specs []cli.CommandSpec) string {
	...
	fmt.Fprintf(out, "`ibkr` is one binary with %d subcommands, listed here in registry order. ", len(specs))
	out.WriteString("`ibkr status` comes first on purpose: when anything else fails, it is the command that says whether the daemon is running and the gateway is reachable. ")
	out.WriteString("Running `ibkr` with no arguments in an interactive terminal opens the full-screen terminal app, which reads the same registry.\n\n")

	out.WriteString("Add `--json` to a data command for machine-readable output instead of a table. ")
	out.WriteString("Commands that accept `--watch` redraw on an interval set by `--rate`, as in `ibkr account --watch --rate 1s`. ")
	out.WriteString("Color is on for terminals and off for pipes; `NO_COLOR=1` turns it off, and `IBKR_COLOR=always` or `IBKR_COLOR=never` forces it either way.\n\n")

	out.WriteString("The flag tables below come from the shared command catalog. ")
	out.WriteString("Run `ibkr <command> --help` for the flags a command parses today, with their help text. That output is the authority if the two ever disagree.\n\n")
	...
	for _, spec := range specs {
		...
		fmt.Fprintf(out, "| [`ibkr %s`](#ibkr-%s) | %s | `%s` | %s |\n",
...
func renderCommand(out *strings.Builder, spec cli.CommandSpec) {
	...
	if len(spec.Flags) == 0 {
		fmt.Fprintf(out, "*The catalog records no flags. Run `ibkr %s --help` for what the command parses.*\n\n", spec.Name)
		return
	}
...
// (`ibkr order place`), so the summary reports the strictest reachable class
