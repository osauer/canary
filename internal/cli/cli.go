...
func init() {
	commands = []Command{
		...
		{"market", "Observed quotes, price history and a daily price/breadth/volume tape", "canary market [--symbol SYMBOL --range 1D] [--json] | canary market tape [--sessions 5] [--explain] [--json]", runMarket},
		...
		{"brief", "Combined post- and pre-trade operator brief with disclosed source degradation", "canary brief [--details] [--json]", runBrief},
		...
		{"rules", "Advisory daily trading rule checklist, hardest breach first, against limits you can set", "canary rules [--all] [--symbol SYM] [--json] | canary rules history [--since YYYY-MM-DD|RFC3339] [--until YYYY-MM-DD|RFC3339] [--rule ID] [--limit N] [--json] | canary rules policy [--json] | canary rules policy set KEY=VALUE… [--file PATH] | canary rules policy reset KEY…|--all [--file PATH]", runRules},
		{"policy", "Risk constitution: effective limits, capital/drawdown state, overrides (human-only writes)", "canary policy show [--explain] [--json] | canary policy capital-event deposit|withdrawal [--amount F] [--effective-at TIME] [--note S] | canary policy capital-event reconcile [--report ID] | canary policy override --control KEY --reason S --hours N | canary policy reset-drawdown --reason S | canary policy correct-peak (--from-statements|--peak F) --reason S | canary policy default protection|opportunity|rulebook", runPolicy},
		...
		{"reporting", "IBKR statement reporting status and performance series for Recon and Edge", "canary reporting status|performance [--json]", runReporting},
		{"proposals", "Daemon-owned close/reduce-only protection proposals", "canary proposals status|refresh|list|preview|submit|reduce|request-stop|ignore|veto [--json]", runProposals},
		...
		{"setup", "Configure a local client, app host, or shared broker reporting", "canary setup [claude-desktop|app|reporting]", nil},                               // dispatched in cmd/canary/main.go — local setup workflow
