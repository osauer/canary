...
func init() {
	commands = []Command{
		...
		{"portfolio", "Current signed portfolio composition with coverage", "canary portfolio [--json]", runPortfolio},
		...
		{"reporting", "Shared IBKR statement reporting status for Recon and Edge", "canary reporting status|performance [--json]", runReporting},
		{"proposals", "Daemon-owned close/reduce-only protection proposals", "canary proposals status|refresh|list|preview|submit|reduce|request-stop|ignore [--json]", runProposals},
