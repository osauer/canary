...
func HelpGroups() []HelpGroupSpec {
	return []HelpGroupSpec{
		...
		{Group: GroupMarkets, Title: "Markets", Tagline: "broad-market regime and named-symbol technical evidence"},
...
func catalogExtras() map[string]CommandSpec {
	return map[string]CommandSpec{
		...
		"market":        {Group: GroupMarkets, Flags: flags(boolFlag("watch"), valueFlag("symbol", nil), valueFlag("range", []string{"1D", "5D", "1M", "6M", "YTD", "1Y", "5Y"}), valueFlag("sessions", nil), boolFlag("explain"), boolFlag("history"), valueFlag("before", nil), valueFlag("exchange", nil), valueFlag("type", []string{"STK", "IND", "CASH"}), valueFlag("currency", nil), boolFlag("json")), Subcommands: subcommands("tape")},
		...
		"rules":         {Group: GroupDesk, Brief: "Advisory daily rule checklist, hardest breach first; your limits via rules policy", Flags: flags(boolFlag("all"), valueFlag("symbol", nil), valueFlag("since", nil), valueFlag("until", nil), valueFlag("rule", nil), valueFlag("limit", nil), valueFlag("file", nil), boolFlag("json")), Subcommands: subcommands("history", "policy")},
		...
		"stress":        {Group: GroupDesk, Brief: "Portfolio stress and its full risk evidence", Flags: flags(boolFlag("details"), boolFlag("json"))},
		"brief":         {Group: GroupDesk, Brief: "Prioritized desk brief; full evidence with --details", Flags: flags(boolFlag("details"), boolFlag("json")), Guard: GuardReadOnly},
		"edge":          {Group: GroupDesk, Brief: "Broker-truth review of past trading decisions", Flags: flags(valueFlag("window", []string{"90d", "365d"}), valueFlag("horizon", []string{"1", "5", "20"}), valueFlag("limit", nil), valueFlag("change", nil), valueFlag("option", nil), boolFlag("json")), Guard: GuardReadOnly},
		"proposals":     {Group: GroupDesk, Flags: flags(valueFlag("quantity", nil), valueFlag("timeout", nil), boolFlag("fast-path"), boolFlag("prepared-ref-stdin"), valueFlag("reason", nil), valueFlag("percent", []string{"25", "50", "75", "100"}), valueFlag("con-id", nil), boolFlag("include-hedges"), boolFlag("portfolio"), boolFlag("submit"), boolFlag("json")), Subcommands: []SubcommandSpec{{Name: "status", Guard: GuardReadOnly}, {Name: "refresh", Guard: GuardReadOnly}, {Name: "list", Guard: GuardReadOnly}, {Name: "preview", Guard: GuardReadOnly}, {Name: "prepare", Guard: GuardReadOnly}, {Name: "prepared-status", Guard: GuardReadOnly}, {Name: "submit", Guard: GuardConfirm}, {Name: "reduce", Guard: GuardConfirm}, {Name: "request-stop", Guard: GuardLocal}, {Name: "ignore", Guard: GuardLocal}, {Name: "veto", Guard: GuardLocal}}, Guard: GuardConfirm},
		...
		"policy":        {Group: GroupDesk, Brief: "Risk constitution and policy files: limits, capital and drawdown state", Flags: flags(boolFlag("explain"), valueFlag("amount", nil), valueFlag("effective-at", nil), valueFlag("note", nil), valueFlag("control", nil), valueFlag("reason", nil), valueFlag("hours", nil), valueFlag("report", nil), valueFlag("peak", nil), boolFlag("from-statements"), boolFlag("dry-run"), valueFlag("apply-plan", nil), valueFlag("config", nil), boolFlag("json")), Subcommands: []SubcommandSpec{{Name: "show", Guard: GuardReadOnly}, {Name: "capital-event", Guard: GuardConfirm}, {Name: "override", Guard: GuardConfirm}, {Name: "reset-drawdown", Guard: GuardConfirm}, {Name: "correct-peak", Guard: GuardConfirm}, {Name: "default", Guard: GuardLocal}, {Name: "ensure", Guard: GuardLocal}}, Guard: GuardReadOnly},
		"recon":         {Group: GroupDesk, Brief: "Post-trade reconciliation against the capital ledger", Flags: flags(boolFlag("refresh"), valueFlag("line", nil), valueFlag("reason", nil), valueFlag("since", nil), valueFlag("until", nil), valueFlag("limit", nil), boolFlag("json")), Subcommands: []SubcommandSpec{{Name: "status", Guard: GuardReadOnly}, {Name: "show", Guard: GuardReadOnly}, {Name: "backtest", Guard: GuardReadOnly}, {Name: "equity", Guard: GuardReadOnly}, {Name: "dismiss", Guard: GuardConfirm}}, Guard: GuardReadOnly},
		"reporting":     {Group: GroupDesk, Brief: "Shared statement-reporting setup and health", Flags: flags(boolFlag("json")), Subcommands: subcommands("status", "performance"), Guard: GuardReadOnly},
		...
		"setup":    {Group: GroupSystem, Flags: flags(boolFlag("accept-unproved"), boolFlag("no-restart")), Subcommands: []SubcommandSpec{{Name: "claude-desktop", Guard: GuardLocal}, {Name: "app", Guard: GuardLocal}, {Name: "reporting", Guard: GuardLocal}}, Guard: GuardLocal},
