...
//
// While the risk constitution's drawdown brake is engaged, long option premium
// is reduced to the share of declared risk capital the owner wrote into
// [buckets.budget_reduction]: per line first, then the total, in whole
// contracts, largest unrealised loss first. Protection legs are never
...
func (e *proposalEngine) budgetGovernorInput(acct *rpc.AccountResult, now time.Time) budgetGovernorInput {
	...
	return budgetGovernorInput{Constitution: authority.policy, Capital: res.Capital, Unapproved: res.Unapproved}
}
...
type budgetLine struct {
	...
	// gain), the first ordering key of the total pass.
	unitBase  float64
	valueBase float64
	lossBase  float64
	pnlBase   *float64
...
func budgetReductionPlan(policy protectionPolicy, input budgetGovernorInput, pos *rpc.PositionsResult, now time.Time) budgetPlan {
	...
	plan := budgetPlan{status: rpc.TradeProposalBudgetStatus{
		Mode: mode, Shadow: mode == rpc.BudgetReductionModeShadow,
	...
	excess := remaining - plan.totalCap
	if excess > budgetMoneyEpsilon {
		order := make([]int, len(plan.lines))
		for i := range order {
			order[i] = i
		}
		slices.SortStableFunc(order, func(a, b int) int {
			la, lb := plan.lines[a], plan.lines[b]
...
func budgetLegIsProtection(cfg protectionTrailOptionPolicy, row rpc.PositionView, pos *rpc.PositionsResult, legs map[int]bool, ambiguous map[string]bool, pol risk.RulebookPolicy, now time.Time) bool {
	if optionExitPurpose(cfg, row, pos, legs, ambiguous, now) == "protection" {
		return true
	}
