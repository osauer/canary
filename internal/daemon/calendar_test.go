...
func TestCalendarWindowsReachConsumerAndAsianLoggingScope(t *testing.T) {
	...
	// Tokyo/Hong Kong open while US and Xetra are closed.
	at, _ := time.Parse(time.RFC3339, "2026-09-14T02:00:00Z")
	if !testLogMarketsOpen(at) {
		t.Fatal("default gateway logging scope omitted Asia")
	}
}
