...
var Tools = []Tool{
	...
	},
	{
		...
		Description: strings.Join([]string{
			...
			"`fingerprint` is the semantic alert identity for monitor dedupe/recovery; `source_fingerprints.account`, `.positions`, `.regime`, and `.market_events` record the classified input buckets consumed by this canary run; `source_health[]` records each source's `as_of`, freshness/degraded/stale/partial status, confidence, max-age cadence, semantic-bucket fingerprint stability, durable `refresh_state`/`next_attempt`, and redacted typed `last_failure` (`code`, `stage`, `failed_at`, `retryable`). Respect retry/backoff/not-due state and do not infer failure classes from `notes`.",
