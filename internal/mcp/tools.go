...
var Tools = []Tool{
	...
	},
	{
		...
		Description: strings.Join([]string{
			...
			"NOT for account/portfolio action, trade selection, hedging, sizing, execution, or a probability forecast — use `ibkr_stress` when the user needs market weather combined with held portfolio shape, and use `ibkr_positions` / `ibkr_account` for held-risk inspection.",
			...
			"Per-row fields include raw measurements, `status`, `band`, `band_reason`, `thresholds` (heuristic + pending_backtest), `eligibility` (`eligible`, `latched`, `reasons[]`), `freshness` (`class` fresh/not_due/overdue + served `max_age_seconds`), `as_of` (`label`, freshness/source/time/date), `streak` (consecutive NY trading sessions in band), and per-scalar `*_quality` provenance.",
	...
	{
		Name:  "ibkr_stress",
		Title: "IBKR Portfolio Stress",
		Description: strings.Join([]string{
			"Live stateless portfolio stress read for scheduled checks every few minutes: it combines broad-market weather from `ibkr_regime` with the user's current portfolio shape.",
			...
			"NOT for account-only risk such as margin breach, cash, buying power, or daily P&L in isolation — use `ibkr_account` or a dedicated account-risk workflow for that. Stress evidence may include margin/P&L facts, but the headline stress action is gated by market confirmation plus portfolio fit.",
