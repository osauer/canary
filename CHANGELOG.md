...
- **A red dealer-gamma reading can no longer be talked out of counting by the smaller index.** The combined SPY+SPX row decides its colour by weighting each index by the gamma actually sitting on it, so SPX normally decides. The depth test that then asks whether the red is deep enough to count towards stress used a plain average of the two gaps instead, and a calm SPY cancelled out an alarming SPX: a book reading SPX 2.5% below its zero-gamma with ten times SPY's gamma on it came out at 0.05% and was dropped as too shallow. Both steps now weigh the two indexes the same way, so the index that sets the colour is the index that decides whether it counts. Same reading, same thresholds — the desk simply stops discarding a red it had already recognised.
