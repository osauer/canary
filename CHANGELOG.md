...
- **Starting this version clears most of Canary's stored analytical history, once.** Canary's beta releases kept every raw market refresh and every analytical event forever, and nothing read them back. The first start on this version removes them: stored market-refresh observations, rule, stress and alert analytical events, superseded dealer-gamma computes, and all but the two most recent regime readings. What the desk runs on is kept — current state, broker and order evidence, preview-token tombstones and order-ID floors, capital, risk-policy, statement and reconciliation records, earnings-identity proofs, submitted and ignored proposals, and anything Canary does not recognise. This is a one-time reset of beta history, not a retention setting: nothing is trimmed on any later start. **Export anything you want to keep before upgrading.** The upgrade writes a safety copy of the new database, not the old one, so removed history is not recoverable from it afterwards.
...
- **Four more regime indicators can now say how bad a red reading is.** Credit spreads, funding stress and USD/JPY reported no depth at all, and dealer gamma's fast path sat exactly on its own red boundary, so on those four a barely-red reading and a severe one were indistinguishable. Each now has a saturation point derived from its own bands — high-yield spreads at 6.5%, funding at 105 basis points, gamma at 4.5% below zero-gamma, and USD/JPY at 7% weekly yen strengthening, the one value set by the August 2024 carry unwind rather than the rule. Which readings confirm stress is unchanged; the desk simply records how deep each red was.
...
- **A regime cluster getting worse can no longer make the alarm quieter.** Two of the stress rules counted the clusters sitting at yellow. A cluster that deteriorated from yellow to red left that count, and if its red was not yet deep or persistent enough to confirm, it joined no other count either — so the rule stopped matching and the reported stage fell. On a 5% down day with VIX up 40%, breadth and gamma amber and everything else calm, the desk read confirmed stress; moving gamma alone from amber to red made it read early warning instead. Nothing had improved. Those rules now count clusters at amber *or worse*, a number that cannot fall when the market deteriorates. This makes confirmed stress reachable in more situations than before, which is the intended direction: the rule exists to catch a bad tape with corroborating weakness, and a red corroborates at least as well as an amber.
...
- **Alert history no longer grows without bound.** Every heartbeat and hold evaluation appended a stored event, so the alert log grew continuously whether or not anything changed. Only genuine lifecycle transitions are recorded now; routine re-evaluations stay in the current alert record where they were already available.
