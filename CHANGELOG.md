...
- **The rulebook evaluates again on a TWS login that holds several accounts.** v2.6.0 got positions and balances back for such a desk, but `canary rules` still reported input health `account: unavailable` and returned all eleven account-dependent rules as unknown — exposure caps, the premium cap, cash-sell-only, the theta budget, expiry runway, the earnings rules, hedge integrity, exit discipline and FX exposure. Two faults stacked. The account-summary request is issued for the login rather than for one account, so it answers with rows for every account the login carries; Canary read the first sibling row as proof the read had been misrouted and discarded the whole snapshot, which made that request fail on every evaluation rather than occasionally. The daemon then fell back to its streaming cache, and the cache labelled its snapshot with the raw account list instead of the pinned account — a label the rulebook refuses, because it names two accounts where the desk pinned one. The request now completes: a sibling row from the same login is expected traffic for the request that was sent and is passed over, while the pinned account's own rows finish the snapshot. An account the login does not manage still rejects the whole set, so a genuinely misrouted read keeps failing closed. One deliberate limit: such a login gets no currency-ledger rows until each row can be attributed to its own account, because the ledger row the broker returns for a whole-login request carries no account of its own and reporting a sibling's currency exposure as the pinned account's would be worse than omitting it. Every other field in the snapshot carries its own concrete account and is unchanged, and a single-account login is unaffected throughout. A second fault, older than v2.6.0, is closed here too, and it is worth describing because its symptom was alarming: an account-summary request that timed out was deregistered while the broker's reply was still in flight, and a late row naming one concrete account could then overwrite a session identity that had never been concrete — so a multi-account session could rebind itself to a sibling. The desk showed that as `gateway_account_mismatch` and refused every broker write for as long as it stood. The refusal was the whole of it, and that is worth being exact about: the mismatch check treats a configured account that is not concrete as a mismatch too, so an unpinned multi-account login was refused rather than quietly allowed, and the only rebind that could poison the field was a rebind to a sibling — precisely the case the refusal caught. It denied orders; it never misrouted one. A session identity is no longer seeded from a login already known to carry several accounts, so the rebind cannot happen. What remains is narrower and stated so it does not look fixed: the timeout window itself is still open, so a late reply is still unregistered traffic — it simply can no longer take the session with it. Ten of the eleven rules evaluate, not all eleven: FX exposure stays unknown on a multi-account book holding a non-base leg, because it needs the currency-ledger rows this release deliberately withholds, and reporting a base-only reading from an empty report would be a guess rather than an answer. Multi-account support is not finished either: with no pin set there is still no account to name, and such a login's currency breakdown is still absent.

- **A multi-account desk can place orders again.** IBKR terminates its account list with a trailing comma, and the check that compares the pinned account against the connected session read that empty final entry as a malformed member — so a pin that genuinely was one of the login's accounts came back as a mismatch, and the desk refused every broker write with `gateway_account_mismatch`. Without the fixes above it was invisible, because such a login could not get that far; with them, it would have been the next thing in the way. A trailing comma is now read as the list syntax it is. Everything the check refused before, it still refuses: a list of only empty entries, an aggregate member such as `All` sitting beside a real account, a pin absent from the login, and a configured account that is itself a list.

- **Earnings dates resolve from Nasdaq again.** The parser required the announcement's date to arrive inside a nested status envelope and treated a top-level status beside it as a contradiction. The endpoint serves the opposite shape, and only that shape, for dated and undated announcements alike — so every published date landed on the contradiction check and was thrown out. No date had resolved from Nasdaq since that contract shipped, which left held names with a perfectly good published date sitting in the earnings rule's unknown list. The parser now reads the envelope the endpoint actually serves, and accepts both day spellings it uses: zero-padded for a scheduled date, unpadded for an estimate. Nothing was loosened to get there — the retired envelope, both statuses arriving together, a missing or mistyped top-level status code, and the rest of the announcement grammar all still fail closed.

- **A treasury or bond no longer shows the daily move of the stock that shares its ticker.** Every non-option holding was priced by asking for a quote on its bare symbol as if it were a stock, so a treasury issue symbolled `T` was decorated with AT&T's quote — a bill marked at 98.5 against AT&T's close of 28 printed a daily move of about 252%, and a phantom daily P&L some seventy times what the holding was actually worth. The quote pinned beside the holding was wrong — and so was the value Canary put on the holding itself, which is the next entry. This became reachable only in v2.6.0: before it, the multiplier check discarded those rows before anything could price them, so fixing that fault is what exposed this one. Bonds, bills, funds and futures now keep the broker's own mark and carry no quote context and no day change, rather than a confident wrong one. Equities are unaffected.

### Fixed

- **The account Canary reports as connected is one account, never a list.** A login holding several accounts advertises all of them, and that whole comma-joined string was served as `connected_account` — so the app and the answers AI clients give named every account at once, on a desk scoped to one. The terminal already guarded its own display, which only widened the gap: the same field carried different things depending on where you read it. It now carries the account the session is actually scoped to — the advertised code when the login holds one, the configured pin when it holds several — and nothing at all when a multi-account login has no pin set, because then there genuinely is no account to name. The daemon's own scope checks still read the full advertised value, so nothing about which account is trusted for orders has changed, and a single-account login sees exactly what it saw before. Naming the whole inventory as well would need a second field, and nothing asks for one yet.
- **A bond, bill, fund or future no longer reports a market value roughly a hundred times too large.** IBKR quotes those instruments as a percentage of par against a face-value quantity, and Canary multiplied the two — so a bill marked at 98.5 on 10,000 face was published as about 985,000 instead of 9,850, in the contract currency and in base currency alike, on the position list, in the app and in the answers AI clients give. The broker sends its own market value on the same frame and that is now the figure reported; it is validated on arrival, and a frame that omits it or sends a non-finite one discards the whole generation, so nothing is guessed and there is nothing to fall back to. Equities and options are unchanged, because for them the product and the broker's figure are the same number — which is why this stayed hidden. Like the day-change fault above it became reachable only in v2.6.0, when those rows began being published at all. No rule verdict moves: a non-equity holding cannot form an underlying group, so per-name exposure, concentration and the trim proposals never read these rows. One number this deliberately does not reach, said plainly so it does not look covered: the portfolio's aggregate dollar delta is computed from mark times quantity in its own right rather than from this field, so a percent-of-par holding still enlarges it, and the portfolio derisk sweep sizes its trim from that aggregate. Preview a sweep on such a book and read the per-leg quantities before submitting. Closing that means deciding what delta a fixed-income leg contributes at all, which is a policy question taken separately.
- **The FX-exposure rule no longer reports a clean 0% on a book it could not measure.** Deciding whether an absent currency report may be read as a genuinely single-currency book was done by walking the grouped view of holdings — but only equities form a group, so a bond, bill, fund, future or cash leg was invisible to it. A desk holding a foreign bond therefore looked same-currency, and the rule asserted a corroborated pass over an exposure nobody had measured. Every held leg is now inspected regardless of grouping. This needed no multi-account login; one foreign bond was enough.
- **Option-premium, extrinsic-budget and exit-discipline rules no longer read a foreign leg's premium as if it were already in your base currency.** When no exchange rate is available for a leg, its raw contract-currency figure stands in. That is right only when the leg is already in base, and otherwise wrong by the exchange rate in whichever direction the pair runs — so an understating pair quietly kept a line under its cap, and in the two rules where that number decides whether an unmeasurable leg is *disclosed at all*, it suppressed the disclosure. Such a leg is now marked at the point of substitution: the premium rule reports unknown rather than a pass it did not earn, the other two list the leg unconditionally, and no lower bound is built on a substituted figure. A measured breach is never downgraded by an unrelated leg's missing rate, and the unmeasurable leg stays named on the row either way. This release is what makes it reachable — it repairs the account health that had been keeping these rules unknown on a multi-account login while still withholding that login's currency rows.
- Two holdings that share an underlying no longer report a portfolio percentage that counts only one of them. The group's own market value added up every row, but the base-currency total — the figure behind concentration, and the one the rulebook and the trim proposals both read — was derived from a single representative row. A desk holding one name across two currencies or two issues therefore looked smaller than it was, which is the direction that matters: an exposure rule could pass a name that should have tripped it, and the engine would decline a trim it owed. Every member row now counts toward the base-currency totals. One gap stays open and is not fixed here: the group still displays one of those rows rather than all of them.
- Percent-of-portfolio figures no longer use a sibling account's balance as the denominator. The typed account-summary path was corrected in v2.6.0's follow-up, but a second, untyped reader of the same unattributed cache was left open, and it supplied the net-liquidation figure that every percentage divides by. It now applies the same admissibility rule as the typed path, so on a multi-account login the value is simply absent and the existing bounded refresh supplies an attributed one instead.
- Unrealised and realised P&L report the account total rather than one currency's slice of it. Where a tag arrived both as the account's own base-currency figure and as per-currency ledger rows, the reader fell back to whichever currency sorted first alphabetically — so a desk holding CHF, EUR and USD saw the CHF slice presented as the account figure. The account's own base-currency row is now preferred, and where several ledger candidates exist with no base row, nothing is reported rather than a guess.
- Futures are no longer understated by a contract multiplier read as 1. A rule that coerced a multiplier of exactly 100 applied to every non-option row, not just the equity rows it was written for.
- `canary status` and the terminal dashboard name the account the session is pinned to. Both read the connected identity first, and on a login holding several accounts that value is the comma-joined list of all of them — so the session line claimed the desk was scoped to every account at once and led with a sibling the operator never pinned. Both surfaces now take that identity only when it is one concrete account and otherwise show the pin. A single-account login is unchanged, including before the pin resolves. An unpinned multi-account login now reads `auto-detect`, which is honest rather than helpful: naming the right account there needs a connect-time line that says how many accounts the login carries, and that is still to come.
- A multi-account login gets a clean `unavailable` from the cached account fallback instead of figures under a label naming two accounts. The shared streaming cache keeps no per-account attribution, and nothing clears it when the account-updates subscription rebinds, so at read time nothing can prove its contents belong to the pin — stamping the pin on it would have traded a fail-closed labelling bug for a wrong-numbers bug. Refusing also stops the fallback from masking the summary failure that sent the read there in the first place, which is what hid the rulebook fault above. Single-account desks are unchanged: their observed identity is the concrete account, and it is the label they already got.

### Security

- **A release can no longer be narrowed on its way out, and its required market gate can no longer be switched off.** v2.6.0 bound publication to one commit and a complete evidence set, but two settings could still be overridden from the command line when the pipeline was started. One chose which platforms to build: narrowing it to a single platform left assembly treating that subset as the whole release, and the inventory was only counted after the tag had already been pushed — so an incomplete release could reach a published tag before anything objected. The other switched off a market-data check the release smoke requires, while the strict-gateway setting beside it was already pinned. Both are now refused when the pipeline starts, from the command line and from the environment alike, along with every other setting a release path reads that could weaken a gate — the branch it lands on, the build tags the smoked binary carries, and the link flags baked into published binaries. The payload inventory is proved by exact name before the tag is pushed as well as before publication, and its expected set is a constant checked against the build matrix so the two cannot drift apart. The previous count-based check would have accepted six correct files alongside four duplicates.
- The identity scan refuses to run on a half-merged working tree rather than reporting a clean pass over it. It listed a conflicted file once per merge stage, so an allowlisted file could be counted several times over — but removing that path showed the worse case: the scan simply passed, attesting that content was clean when the content was not yet decided.
