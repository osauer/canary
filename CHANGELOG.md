...
- **`canary breadth` says which trading session it is showing you.** A reading is one session's close, and the daemon deliberately keeps serving the last good one rather than publishing a partial refresh — so a lane that had stopped producing showed plausible percentages with no date anywhere on screen. The session date is now printed, marked when it is no longer the latest completed session, and carried on the JSON and AI-client surfaces as `stale`. The numbers themselves still print: an old close is real evidence, just not today's.
