...
- **`canary status` no longer calls breadth healthy while its connection is dead.** The breadth row took its state from the main broker connection, which is not the one breadth uses — so throughout the outage above the row read `ready` alongside every other subsystem, and only the data-quality line named the problem. It now reports the second connection breadth actually reads over, and says `degraded` when that one is down while the main one is up, naming how many redials have failed. Start-up is unaffected: the row stays quiet until the first attempt to build that connection has been made.
- **Breadth says which trading session it is showing you, in the app as well as the terminal.** A reading is one session's close, and the daemon deliberately keeps serving the last good one rather than publishing a partial refresh — so a lane that had stopped producing showed plausible percentages with no date anywhere on screen. `canary breadth` now prints the session date and marks it when it is no longer the latest completed session; the JSON and AI-client surfaces carry it as `stale`; and the app's Breadth row names its session too, turning degraded when a newer one is overdue. The numbers themselves still print: an old close is real evidence, just not today's. The ordinary post-close window is exempt — the daily refresh is a full 503-name fetch that takes over an hour, and the row stays clean while it runs rather than going amber every evening.
- **A routine "market data farm connection is OK" notice no longer erases a real connectivity break.** The broker sends farm-level OK notices continuously; any one of them cleared the record of a lost TWS-to-IBKR link. Because a broken link is what marks every data feed unavailable in `canary status`, the next such notice turned a live outage green. Only the notices that actually name the link clear it now.
