...
- **The app host no longer crashes mid-refresh while a paired device is streaming.** A published snapshot shared its sources map with the next refresh cycle's working copy, and a device marshalling one while the other wrote aborted the whole app host, disconnecting every paired device. The published snapshot is now fully its own copy.
