...
- **Weekend and off-session daily P&L is no longer labeled as the last completed session's result.** The figure IBKR serves off-session is the running change since the last regular close, marked at off-session prices, and the brief, terminal and app presented it under a last-session label. It now reads as the broker's since-close value at off-session marks, and the app discloses the P/L frame time beside it. The number itself was always the broker's own; only its label and basis line change. (#20)
