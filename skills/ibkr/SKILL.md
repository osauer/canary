...
| `ibkr recon show` | Post-trade reconciliation report: broker Flex statement flows vs the declared capital-event ledger, with exception categories, statement-source health, and the equity divergence check. `dismiss` and the reconcile sign-off are human-only governance acts — read `show` only from this skill | — |
