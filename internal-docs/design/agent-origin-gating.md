...
1. **CLI** resolves origin once at startup into `cli.Env`.
   `CANARY_AGENT_CONTEXT` can only force the *more restrictive* classification
   (`agent`); there is no env/flag that yields `human-tty`. The retired
   `IBKR_AGENT_CONTEXT` spelling remains detection-only so it cannot bypass
...
6. **Hook layer (client-side, defense in depth, this repo's plugin):**
   ...
   write-verb invocations so read-only `canary … --json | jq` stays usable.
